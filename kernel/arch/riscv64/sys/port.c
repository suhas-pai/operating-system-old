...
uint8_t port_in8(const port_t port) {
    ...
    asm volatile("lb %0, 0(%1)" : "=r"(result) : "r"(port) : "memory");
    io_br_fence();
...
uint16_t port_in16(const port_t port) {
    ...
    asm volatile("lh %0, 0(%1)" : "=r"(result) : "r"(port) : "memory");
    io_br_fence();
...
uint32_t port_in32(const port_t port) {
    uint32_t result = *(volatile uint32_t *)port;
...
uint64_t port_in64(const port_t port) {
    ...
    asm volatile("ld %0, 0(%1)" : "=r"(result) : "r"(port) : "memory");
    io_br_fence();
...
void port_out8(const port_t port, const uint8_t value) {
    io_bw_fence();
    asm volatile("sb %0, 0(%1)" :: "r"(value), "r"(port) : "memory");
}

void port_out16(const port_t port, const uint16_t value) {
    io_bw_fence();
    asm volatile("sh %0, 0(%1)" :: "r"(value), "r"(port) : "memory");
}

void port_out32(const port_t port, const uint32_t value) {
    io_bw_fence();
    asm volatile("sw %0, 0(%1)" :: "r"(value), "r"(port) : "memory");
}

void port_out64(const port_t port, const uint64_t value) {
    io_bw_fence();
    asm volatile("sd %0, 0(%1)" :: "r"(value), "r"(port) : "memory");
}