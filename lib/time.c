...
uint8_t hour_12_to_24hour(const uint8_t hour, const bool is_pm) {
    /* Use modulo to wrap hour 12 to hour 0 */
    return (is_pm) ? ((hour % 12) + 12) : (hour % 12);
}

uint8_t hour_24_to_12hour(const uint8_t hour) {
    const uint8_t result = (hour % 12);
    return (result != 0) ? result : 12;
}

bool hour_24_is_pm(const uint8_t hour) {
    return (hour >= 12);
}
...
struct string_view weekday_to_sv(const enum weekday day) {
    switch (day) {
...
struct string_view month_to_sv(const enum month month) {
    switch (month) {
...
enum weekday sv_to_weekday(const struct string_view sv) {
...
enum month sv_to_month(const struct string_view sv) {
#define RETURN_IF_EQUALS(name)                                                 \
    do {                                                                       \
        if (sv_equals(sv, VAR_CONCAT(SV_, name))) {                            \
...
enum month sv_abbrev_to_month(const struct string_view sv) {
#define RETURN_IF_EQUALS(name)                                                 \
    do {                                                                       \
        if (sv_equals(sv, VAR_CONCAT_3(SV_, name, _ABBREV))) {                 \
            return VAR_CONCAT(MONTH_, name);                                   \
                            ...
                            const bool in_leap_year)
{
    ...
    const uint16_t lookup_table[MONTH_COUNT] = {
        0,   /* January */
        31,  /* February */
        59,  /* March */
        90,  /* April */
